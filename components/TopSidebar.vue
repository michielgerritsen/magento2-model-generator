...
export default Vue.extend({
  computed: {
    ...
    includeNonMergable: {
      get() {
        return this.$store.state.download.includeNonMergable
      },
      set(value) {
        this.$store.commit('download/setIncludeNonMergable', value)
      }
    },
  ...
  methods: {
    download() {
      ...
      if (!this.includeNonMergable) {
        files = files.filter((file) => file.isMergeable())
      }
